(function() {
  function getDataSet() {
    ...
    var interestRate = (output.interestRate = parseFloat(
      $("#interestRate").val()
    ));
    var paymentsPerYear = (output.paymentsPerYear = parseInt(
      $("#paymentsPerYear").val()
    ));
    var years = (output.years = parseInt($("#years").val()));
    var numberOfPayments = (output.numberOfPayments = paymentsPerYear * years);

    var payment = (output.payment = pmt(
    ...
    output.schedule = computeSchedule(
      loanAmount,
      interestRate,
      paymentsPerYear,
      years,
  ...
  function reloadTable(ds) {
    ...
    $("#schedule").empty();
    $("#schedule").html(
      '<table cellpadding="0" cellspacing="0" border="0" class="display table" id="schedule_table"></table>'
    );
